...
const Readings = ({ onViewAssociateDetails }) => {
    ...
    const createOrUpdateInvoice = async (batch, readingData) => {
        ...
        const invoiceData = { associateId: readingData.associateId, periodId: readingData.periodId, period: period.billingPeriodName, consumption: readingData.consumption, amountDue: parseFloat(amountDue.toFixed(2)), invoiceDate: new Date().toISOString().split('T')[0], previousReadingValue: readingData.previousReading, latestReadingId: readingData.id };
    ...
    const handleToggleSelect = (associateId) => {
        setSelectedAssociates(prev => {
            ...
            if (newSet.has(associateId)) newSet.delete(associateId);
            else newSet.add(associateId);
            return newSet;
        });
    };
